...
const PRESETS = [
  ...
  { label: "Ontem", get: () => ({ de: somarDias(hojeISO(), -1), ate: somarDias(hojeISO(), -1) }) },
  { label: "Este mês", get: () => {
      const hoje = new Date();
  ...
  { label: "Próximos 30 dias", get: () => ({ de: hojeISO(), ate: somarDias(hojeISO(), 30) }) },
  { label: "Próximos 60 dias", get: () => ({ de: hojeISO(), ate: somarDias(hojeISO(), 60) }) },
  { label: "Próximos 90 dias", get: () => ({ de: hojeISO(), ate: somarDias(hojeISO(), 90) }) },
];

export function PeriodoDropdown({ label }: { label: string }) {
  const [open, setOpen] = useState(false);
  const [customOpen, setCustomOpen] = useState(false);
  const router = useRouter();

  function aplicar(de: string, ate: string) {
    router.push(`/capacidade?de=${de}&ate=${ate}`);
    setOpen(false);
    setCustomOpen(false);
  }
    ...
    <div className="relative w-full sm:w-auto">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-50 mt-2 w-[220px] overflow-hidden rounded-[14px] border border-[var(--color-border-soft)] bg-gradient-to-b from-[var(--color-card-from)] to-[var(--color-card-to)] p-1.5 shadow-xl">
            {PRESETS.map((p) => (
              <button
                key={p.label}
                onClick={() => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          ...
          >
            ...
            <label className="flex flex-col gap-1">
              ...
              <DatePicker name="de" required defaultValue={hojeISO()} />
            </label>
            <label className="flex flex-col gap-1">
              <span className="text-[12px] text-[var(--color-text-muted)]">Até</span>
              <DatePicker name="ate" required defaultValue={somarDias(hojeISO(), 30)} />
            </label>
