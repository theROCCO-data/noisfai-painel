...
const PRESETS = [
  ...
  { label: "Últimos 90 dias", get: () => ({ de: somarDias(hojeISO(), -89), ate: hojeISO() }) },
  {
    label: "Este mês",
    get: () => {
      const hoje = new Date();
...
export function PeriodoDropdown({ label, de, ate }: { label: string; de: string; ate: string }) {
  const [open, setOpen] = useState(false);
  const [customOpen, setCustomOpen] = useState(false);
  const router = useRouter();

  function aplicar(deVal: string, ateVal: string) {
    router.push(`/analises?de=${deVal}&ate=${ateVal}`);
    setOpen(false);
    setCustomOpen(false);
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          ...
          >
            ...
            <label className="flex flex-col gap-1">
              ...
              <DatePicker name="de" required defaultValue={de} />
            </label>
            <label className="flex flex-col gap-1">
              <span className="text-[12px] text-[var(--color-text-muted)]">Até</span>
              <DatePicker name="ate" required defaultValue={ate} />
            </label>
