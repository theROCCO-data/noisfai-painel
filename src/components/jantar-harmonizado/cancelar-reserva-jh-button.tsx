...
import { cancelarReservaJH } from "@/lib/data/jantar-harmonizado-actions";
import { toast } from "@/lib/toast";
import { useEscapeClose } from "@/hooks/use-escape-close";

export function CancelarReservaJHButton({ reservaId, nome }: { reservaId: number; nome: string }) {
  ...
  function confirmar() {
    ...
    startTransition(async () => {
      const result = await cancelarReservaJH(reservaId, motivo);
      if (result.ok) {
        fechar();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={fechar}>
          ...
          >
            ...
            <p className="text-[13.5px] leading-[1.7] text-[var(--color-text-secondary)]">
              Tem certeza que quer cancelar a reserva de <span className="font-semibold text-[var(--color-text-primary)]">{nome}</span> no
              Jantar Harmonizado? A vaga volta pra cota da edição.
            </p>
            ...
            <label className="flex flex-col gap-2.5">
              ...
              <input
                ...
                placeholder="Cliente desmarcou, teste interno..."
